...
public interface StockRepository extends CrudRepository<Stock, Long> {
     ...
     * SELECT
              ...
              NVL(ROUND(CAST(SUM(stock.qty_on_hand) AS REAL) / (SUM(sales_order_line.qty) / EXTRACT(WEEK FROM CURRENT_DATE())), 2),0) AS "weeksOfInventory"
            FROM sales_order so
            ...
            SELECT TOP 5
              ...
              NVL((SELECT SUM(stock.qty_on_hand) FROM stock WHERE stock.item_id = item.id),0) as "qty on hand",
              FROM sales_order so
              ...
              SELECT TOP 5
              item.description as "description",
              SUM(sales_order_line.qty) /  EXTRACT(WEEK FROM CURRENT_DATE()) as "average week sales",
              NVL((SELECT SUM(s.qty_on_hand) FROM stock s WHERE s.item_id = item.id),0) as "qty on hand",
              NVL(ROUND((SELECT SUM(s.qty_on_hand) FROM stock s WHERE s.item_id = item.id) / CAST((SUM(sales_order_line.qty) /  EXTRACT(WEEK FROM CURRENT_DATE())) AS REAL),2),0)  as "weeks of inventory"
              FROM sales_order so
              INNER JOIN sales_order_line ON sales_order_line.sales_order_id = so.id
              INNER JOIN item_price ON sales_order_line.item_id = item_price.item_id
              INNER JOIN item ON sales_order_line.item_id = item.id
    ...
    // public List<Stock> findByItemIdAndQty(@Param("itemId") Long itemId, @Param("qty") int qty);
